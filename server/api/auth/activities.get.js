...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.log('Activities API - WHERE conditions:', whereConditions)
    console.log('Activities API - Query params:', queryParams)
      ...
      if (type === 'all') {
        ...
        if (category) {
          ...
          searchConditions.push(`(
            (a.activity_type = 'post' AND b.category = ? AND (b.title LIKE ? OR b.content LIKE ?)) OR 
            (a.activity_type = 'comment' AND d.category = ? AND c.comments_content LIKE ?)
          )`)
          ...
          searchConditions.push(`(
            ...
            (a.activity_type = 'comment' AND c.comments_content LIKE ?)
          )`)
          const searchTerm = `%${search.trim()}%`
        ...
        // 게시글 탭: 게시글 제목/내용 검색
        if (category) {
          searchConditions.push(`b.category = ? AND (b.title LIKE ? OR b.content LIKE ?)`)
          const searchTerm = `%${search.trim()}%`
        ...
        // 댓글 탭: 댓글 내용 검색
        if (category) {
          searchConditions.push(`d.category = ? AND c.comments_content LIKE ?`)
          const searchTerm = `%${search.trim()}%`
          queryParams.push(category, searchTerm)
        } else {
          searchConditions.push(`c.comments_content LIKE ?`)
          const searchTerm = `%${search.trim()}%`
    ...
    if (searchConditions.length > 0) {
      countQuery += ` LEFT JOIN laon_tbl_board b ON a.board_id = b.bno AND a.activity_type = 'post'
                     LEFT JOIN laon_tbl_comments c ON a.comment_id = c.comment_id AND a.activity_type = 'comment'
                     LEFT JOIN laon_tbl_board d ON a.board_id = d.bno AND a.activity_type = 'comment'
    ...
    const sqlQuery = `
      SELECT 
        ...
        CASE 
          ...
          WHEN a.activity_type = 'comment' THEN 
            CASE 
          ...
          WHEN a.activity_type = 'like' THEN 
            CASE 
              WHEN a.comment_id IS NOT NULL THEN '댓글'
              ELSE 
                CASE 
                  WHEN b.title IS NOT NULL THEN b.title
                  ELSE '게시글이 삭제되었습니다'
        ...
        CASE 
          ...
          WHEN a.activity_type = 'comment' THEN c.comments_content
          WHEN a.activity_type = 'like' THEN 
            CASE 
              WHEN a.comment_id IS NOT NULL THEN c.comments_content
              ELSE b.content
            END
          ELSE NULL
        END as content,
        CASE 
          WHEN a.activity_type = 'post' THEN b.category
          WHEN a.activity_type = 'comment' THEN d.category
          WHEN a.activity_type = 'like' THEN b.category
          ELSE NULL
        END as category,
        c.parent_id as parentId
      FROM laon_tbl_activities a
      LEFT JOIN laon_tbl_board b ON a.board_id = b.bno
      LEFT JOIN laon_tbl_comments c ON a.comment_id = c.comment_id
      LEFT JOIN laon_tbl_board d ON a.board_id = d.bno
      WHERE ${finalWhereClause}
      ORDER BY a.regdate DESC
      LIMIT ? OFFSET ?`
    
    console.log('Activities API - SQL Query:', sqlQuery)
    console.log('Activities API - SQL Params:', [...queryParams, limit, offset])
